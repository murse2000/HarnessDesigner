...
describe("SolidWorks Routing 패키지", () => {
  it("From-To, CPoint, 경로와 정적 참조 STEP을 함께 만든다", () => {
    const project = createSampleProject("ROUTING TEST");
  ...
  it("등록된 커넥터 원본 STEP을 components 폴더에 포함한다", () => {
    const project = createSampleProject("ROUTING COMPONENT");
    ...
    project.parts.find((part) => part.id === "builtin-molex-33482-4801")!.modelAssetId = "model-j1";
  ...
  it("연결이 없는 하네스는 Routing 패키지를 만들지 않는다", () => {
    const project = createSampleProject("EMPTY ROUTING");
