...
export function LibraryView() {
  ...
  return <div className="library-view">
    ...
    <div className="library-grid">{loading ? <div className="empty-state">공용 부품 목록을 불러오는 중…</div> : loadError ? <div className="empty-state">{loadError}</div> : parts.length ? parts.map((part) => <article key={part.id} onContextMenu={(event) => { event.preventDefault(); setMenu({ x: event.clientX, y: event.clientY, partId: part.id }); }}><PartThumbnail part={part} project={snapshot.project} /><div><strong>{getPartName(part)}</strong><span>{part.manufacturer}</span><p><code>{part.partNumber}</code> · {part.description}</p></div><em>{partCategoryLabel(part.category, locale)}{part.category === "housing" ? ` · ${getPartPinCount(part)}P` : ""}{part.modelAssetId ? " · 3D" : ""}</em><div className="library-card-actions"><code>REV {part.revision}</code>{part.modelAssetId && <button onClick={() => setAligningPart(part)}><Sliders size={10} />3D 정렬</button>}</div></article>) : <div className="empty-state">{category === "all" ? "등록된 공용 부품이 없습니다." : `${partCategoryLabel(category, locale)} 카테고리에 표시할 부품이 없습니다.`}</div>}</div>
