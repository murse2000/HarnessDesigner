...
export function PinMapView() {
  ...
  const menuItems = (): ContextMenuItem[] => {
    ...
    if (!wire) return [];
    return [
      { label: wire.cableRunId ? "케이블 수정" : "핀맵 연결 수정", icon: <Pencil size={12} />, disabled: released, action: () => wire.cableRunId ? openCableRunEditor(wire.cableRunId) : openPinMapEditor(wire.id) },
      { label: "새 핀으로 연결 복제", icon: <Copy size={12} />, disabled: released, action: () => openPinMapEditor(wire.id, true) },
      { label: "전선 참조명 복사", icon: <Clipboard size={12} />, action: () => void navigator.clipboard.writeText(wire.reference) },
      { label: "새 연결 추가", icon: <Plus size={12} />, disabled: released, separatorBefore: true, action: () => openPinMapEditor() },
      { label: "핀맵 연결 삭제", icon: <Trash2 size={12} />, disabled: released, danger: true, action: () => deleteWire(wire.id) },
    ];
  };
  return <div className="data-view"><PanelHeader title={translate(locale, "pinmap")} icon={<Table2 size={13} />} view="pinmap" sessionId={snapshot.sessionId} harnessId={harness.id} actions={<button className="panel-action-button" disabled={released} onClick={() => openPinMapEditor()}><Plus size={11} />연결 추가</button>} /><TableFrame><thead><tr><th>Wire</th><th>From</th><th>Pin</th><th>To</th><th>Pin</th><th>Wire Part</th><th>Color</th><th>Gauge</th><th>Twist</th><th>Notes</th><th aria-label="편집" /></tr></thead><tbody>{harness.conductors.map((wire) => {
    ...
    return <tr key={wire.id} onClick={() => selectEntity(wire.id, "conductor")} onDoubleClick={() => { if (!released) wire.cableRunId ? openCableRunEditor(wire.cableRunId) : openPinMapEditor(wire.id); }} onContextMenu={(event) => { event.preventDefault(); selectEntity(wire.id, "conductor"); setMenu({ x: event.clientX, y: event.clientY, wireId: wire.id }); }}><td><strong>{wire.reference}</strong></td><td>{from?.reference}</td><td>{from?.pins.find((pin) => pin.id === wire.from.pinId)?.number ?? (wire.shieldGroup ? "OPEN" : "")}</td><td>{to?.reference}</td><td>{to?.pins.find((pin) => pin.id === wire.to.pinId)?.number ?? (wire.shieldGroup ? "OPEN" : "")}</td><td><code>{wirePart?.partNumber}</code></td><td><span className={`wire-color wire-color--${wire.color.toLowerCase()}`} />{wire.color}</td><td>{wire.gauge}</td><td>{wire.twistGroup}</td><td>{wire.notes}</td><td className="row-actions"><IconButton title={wire.cableRunId ? "케이블 수정" : "연결 수정"} disabled={released} onClick={(event) => { event.stopPropagation(); wire.cableRunId ? openCableRunEditor(wire.cableRunId) : openPinMapEditor(wire.id); }}><Pencil size={11} /></IconButton><IconButton title="연결 삭제" disabled={released} onClick={(event) => { event.stopPropagation(); deleteWire(wire.id); }}><Trash2 size={11} /></IconButton></td></tr>;
...
export function CutListView() {
  ...
  return <div className="data-view"><PanelHeader title={translate(locale, "cutlist")} icon={<ListChecks size={13} />} view="cutlist" sessionId={snapshot.sessionId} harnessId={activeHarnessId ?? undefined} /><TableFrame><thead><tr><th>Harness</th><th>Wire / Cable</th><th>From</th><th>To</th><th>Part No.</th><th>Color</th><th>Gauge / Cores</th><th className="number">Length ({preferences.lengthUnit})</th><th className="number">From Strip</th><th className="number">To Strip</th><th>Notes</th></tr></thead><tbody>{rows.map((row) => <tr key={row.conductorId} onClick={() => selectEntity(row.conductorId, segmentIds.has(row.conductorId) ? "segment" : "conductor")}><td>{row.harnessNumber}</td><td><strong>{row.reference}</strong></td><td>{row.from}</td><td>{row.to}</td><td>{row.partNumber}</td><td>{row.color}</td><td>{row.gauge}</td><td className="number">{formatLength(row.lengthMm, preferences)}</td><td className="number">{row.startStripLengthMm ?? "—"}</td><td className="number">{row.endStripLengthMm ?? "—"}</td><td>{row.notes}</td></tr>)}</tbody></TableFrame></div>;
}
...
export function ValidationBar() {
  ...
  return <div className="validation-list"><div className="validation-summary"><CircleAlert size={13} />{translate(locale, "validation")}<strong>{errors} 오류</strong><span>{warnings} 경고</span></div>{issues.map((issue) => {
    ...
    return <button key={issue.id} title={issue.details} onClick={() => {
    ...
    }}><span className={`severity severity--${issue.severity}`} /><span>{translate(locale, issue.messageKey)}{issue.details && <small>{issue.details}</small>}{destination.label && <small>{destination.harness?.number} · {destination.label}</small>}</span><code>{issue.code}</code></button>;
  })}</div>;
}
